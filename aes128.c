/**
 * @file aes128.c
 * @brief AES-128 forward cipher (FIPS-197) + CTR mode.
 */
#include <aes128.h>
#include <lib/string.h>

static const uint8_t SBOX[256] = {
    
};

static const uint8_t RCON[10] = { 0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36 };

static uint32_t sub_word(uint32_t w) {
    uint8_t b[4] = { (uint8_t)(w>>24), (uint8_t)(w>>16), (uint8_t)(w>>8), (uint8_t)w };
    for (int i = 0; i < 4; i++) b[i] = SBOX[b[i]];
    return ((uint32_t)b[0]<<24) | ((uint32_t)b[1]<<16) | ((uint32_t)b[2]<<8) | b[3];
}

static uint32_t rot_word(uint32_t w) {
    return (w << 8) | (w >> 24);
}

void aes128_init(aes128_ctx *ctx, const uint8_t key[16]) {
    uint32_t *w = ctx->rk;
    for (int i = 0; i < 4; i++)
        w[i] = ((uint32_t)key[i*4]<<24) | ((uint32_t)key[i*4+1]<<16) |
               ((uint32_t)key[i*4+2]<<8) | key[i*4+3];
    for (int i = 4; i < 44; i++) {
        uint32_t temp = w[i-1];
        if (i % 4 == 0)
            temp = sub_word(rot_word(temp)) ^ ((uint32_t)RCON[i/4 - 1] << 24);
        w[i] = w[i-4] ^ temp;
    }
}

static uint8_t xtime(uint8_t x) {
    return (uint8_t)((x << 1) ^ ((x & 0x80) ? 0x1B : 0));
}
static uint8_t gmul(uint8_t a, uint8_t b) {
    /* only ever called with b in {1,2,3} here */
    if (b == 1) return a;
    if (b == 2) return xtime(a);
    return (uint8_t)(xtime(a) ^ a); /* b == 3 */
}

static void add_round_key(uint8_t state[16], const uint32_t *rk) {
    for (int c = 0; c < 4; c++) {
        state[c*4+0] ^= (uint8_t)(rk[c] >> 24);
        state[c*4+1] ^= (uint8_t)(rk[c] >> 16);
        state[c*4+2] ^= (uint8_t)(rk[c] >> 8);
        state[c*4+3] ^= (uint8_t)(rk[c]);
    }
}

static void sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = SBOX[state[i]];
}

static void shift_rows(uint8_t state[16]) {
    uint8_t t;
    /* row 1: shift left 1 */
    t = state[1];  state[1] = state[5];  state[5] = state[9];  state[9] = state[13]; state[13] = t;
    /* row 2: shift left 2 */
    t = state[2];  state[2] = state[10]; state[10] = t;
    t = state[6];  state[6] = state[14]; state[14] = t;
    /* row 3: shift left 3 (== right 1) */
    t = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = t;
}

static void mix_columns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        uint8_t *s = state + c*4;
        uint8_t s0 = s[0], s1 = s[1], s2 = s[2], s3 = s[3];
        s[0] = (uint8_t)(gmul(s0,2) ^ gmul(s1,3) ^ s2 ^ s3);
        s[1] = (uint8_t)(s0 ^ gmul(s1,2) ^ gmul(s2,3) ^ s3);
        s[2] = (uint8_t)(s0 ^ s1 ^ gmul(s2,2) ^ gmul(s3,3));
        s[3] = (uint8_t)(gmul(s0,3) ^ s1 ^ s2 ^ gmul(s3,2));
    }
}

void aes128_encrypt_block(const aes128_ctx *ctx, const uint8_t in[16], uint8_t out[16]) {
    uint8_t state[16];
    /* column-major: state[c*4+r] = in[c*4+r] (AES byte order is already column-major on the wire) */
    memcpy(state, (void *)in, 16);

    add_round_key(state, ctx->rk + 0);
    for (int round = 1; round <= 9; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, ctx->rk + round*4);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, ctx->rk + 40);

    memcpy(out, state, 16);
}

void aes128_ctr_xor(const aes128_ctx *ctx, uint8_t counter_block[16],
                     const uint8_t *in, uint8_t *out, uint32_t len) {
    uint8_t ks[16];
    uint32_t off = 0;
    while (off < len) {
        aes128_encrypt_block(ctx, counter_block, ks);
        uint32_t chunk = len - off;
        if (chunk > 16) chunk = 16;
        for (uint32_t i = 0; i < chunk; i++)
            out[off+i] = in[off+i] ^ ks[i];
        off += chunk;

        /* increment the 128-bit big-endian counter */
        for (int i = 15; i >= 0; i--)
            if (++counter_block[i] != 0)
                break;
    }
}
