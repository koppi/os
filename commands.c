 ...
 *
 * The boot RAM disk (@c /rd): GRUB loads it from os.iso and it holds the full
 * payload (the shell, the C compiler and its sources, every sample program).
 * A real IDE disk, if present, mounts after it as @c /hda; the floppy (@c /fda)
 * is still reachable by an explicit path, but a bare name should not depend on
 * it — the FDC read path can wedge on a cold motor.
 */
#define DEFAULT_DEVICE "/rd"
