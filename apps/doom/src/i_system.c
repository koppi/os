...
#else
    // koppi-os: stdout really is the console, and saying so keeps I_Error on
    // the plain-text path instead of trying to raise a zenity dialog box.
    return 1;
#endif
#endif
}
...
void I_Quit (void)
{
    ...
    // koppi-os: upstream leaves this to SDL, so without ORIGCODE the quit
    // menu would run the exit hooks and then fall back into the game loop.
    // exit() also runs the shim's own handlers, which release the screen.
    exit(0);
}
