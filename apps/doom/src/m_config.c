...
{
    // koppi-os: there is no home directory and no per-process working
    // directory, so default to the boot RAM disk -- always mounted, always
    // writable -- and let -savedir point somewhere that survives a reboot.
    // The trailing separator matters: callers concatenate a bare filename.
    char *result;
    int p = M_CheckParmWithArgs("-savedir", 1);
    const char *dir = (p > 0) ? myargv[p + 1] : "/rd/";
    size_t len = strlen(dir);
    int need_sep = (len > 0 && dir[len - 1] != DIR_SEPARATOR);

    result = (char *) malloc(len + need_sep + 1);
...
char *M_GetSaveGameDir(char *iwadname)
{
    ...
    else
    {
        ...
        free(topdir);
#else
        // koppi-os: the FAT driver cannot create directories, so save games
        // sit beside the config file rather than in a subdirectory of it.
        // Their names ("doomsav0.dsg", "temp.dsg") are already 8.3.
