...
static void BuildIWADDirList(void)
{
    ...
    // koppi-os: FILES_DIR is the boot RAM disk, which is where the build
    // stages a WAD if one was present. Also look on a hard disk, a floppy
    // and the console's working directory, so a WAD too big for the RAM
    // disk can simply be dropped on a real volume.
    AddIWADDir ("/hda");
    AddIWADDir ("/fda");
    AddIWADDir (".");
